...
public class MsgThread implements Runnable {
	...
	public MsgThread(Socket socket, String serverName, int serverPort) throws IOException { // 생성자
		//super();
		this.socket = socket;
	...
	public void run() {
		try {
			...
			try {
				...
				System.err.println("MsgThread in-4 : " + e.getCause());
				e.printStackTrace();
			} finally {
				sdis = null;
			}
			try {
				...
				System.err.println("MsgThread in-5 : " + e.getCause());
				e.printStackTrace();
			} finally {
				sdos = null;
			}
			try {
				...
				System.err.println("MsgThread in-6 : " + e.getCause());
				e.printStackTrace();
			} finally {
				serverSocket = null;
			}
